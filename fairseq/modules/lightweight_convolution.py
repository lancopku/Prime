...
class LightweightConv1dTBC(nn.Module):
    ...
    def __init__(self, input_size, kernel_size=1, padding_l=None, num_heads=1,
                 weight_dropout=0., weight_softmax=False, bias=False, args=None, cur_attn_type='es'):
        super().__init__()
        self.input_size = input_size
        self.kernel_size = kernel_size
        ...
        self.weight = nn.Parameter(torch.Tensor(num_heads, 1, kernel_size))
        sample_lc = args.sample_lc
        self.sample_lc_sp = args.sample_lc_sp
        if sample_lc and cur_attn_type in args.use_att:
            if self.sample_lc_sp:
                self.weight_linear = nn.Linear(1, num_heads*kernel_size)
            else:
                self.weight_linear = nn.Linear(self.input_size, num_heads * kernel_size)
                # bleu 15.74  valid loss 2.7 因为avg了不同时刻
        else:
            self.weight_linear = None
        if bias:
            self.bias = nn.Parameter(torch.Tensor(input_size))
        else:
            self.bias = None
    ...
    def _forward_unfolded(self, x, incremental_state):
        ...
        if bmm_fp16_support:
            ...
            output = torch.bmm(x_unfold.float(), weight.float()).type_as(weight)  # T*B*H x R x 1
    ...
    def _forward_expanded(self, x, incremental_state):
        ...
        if self.weight_linear:
            if self.sample_lc_sp:
                weight = self.weight_linear(torch.mean(torch.mean(x, dim=0), dim=1).unsqueeze(1)).view(B, H, K)  # B,H,K
            else:
                weight = self.weight_linear(torch.mean(x, dim=0)).view(B, H, K)  # B,H,K
        else:
            weight = self.weight.view(H, K)
        if self.weight_softmax:
            if self.weight_linear:
                weight = utils.softmax(weight, dim=2, onnx_trace=self.onnx_trace).type_as(weight)
            else:
                weight = utils.softmax(weight, dim=1, onnx_trace=self.onnx_trace).type_as(weight)
        if self.weight_linear:
            weight = weight.unsqueeze(0).expand(T, B, H, K).reshape(T*B, H, K).contiguous()
        else:
        ...
        if bmm_fp16_support:
            ...
            output = torch.bmm(weight_expanded.float(), x.float()).type_as(weight)
