...
@register_lr_scheduler('cosine_bound')
class CosineBoundSchedule(FairseqLRScheduler):
    ...
    def __init__(self, args, optimizer):
        ...
        self.up_lr = self.args.lr_up * self.max_lr
        self.low_lr = self.args.lr_low * self.max_lr
    ...
    def add_args(parser):
        ...
        parser.add_argument('--lr_up', default=1.0, type=float,
                            help='cut learning rate')
        parser.add_argument('--lr_low', default=0.2, type=float,
                            help='cut learning rate')
        # fmt: on
    ...
    def step_update(self, num_updates):
        ...
        if self.lr > self.up_lr:
            self.lr = self.up_lr
        if self.lr < self.low_lr:
            self.lr = self.low_lr
        self.optimizer.set_lr(self.lr)
        return self.lr
